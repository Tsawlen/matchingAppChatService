...
func GetAllMessagesForUser(db *gocql.Session, userId int) (*[]dataStructure.ChatMessage, error) {
	...
	for iterator1.Scan(&message.MessageID, &message.UpdatedAt, &message.ChatID, &message.CreatedAt, &message.Message, &message.Read, &message.SendToUser, &message.WrittenByUserID) {
		messages = append(messages, message)
	}
	if errIterator1 := iterator1.Close(); errIterator1 != nil {
		return nil, errIterator1
	}
	for iterator2.Scan(&message.MessageID, &message.UpdatedAt, &message.ChatID, &message.CreatedAt, &message.Message, &message.Read, &message.SendToUser, &message.WrittenByUserID) {
		messages = append(messages, message)
	}
